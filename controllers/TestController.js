...
export default class TestController {
    ...
    static async getAllTests(c) {
        try {
            const tests = await Test.getAllTest(c.env.DB);
            return c.json(tests);
        } catch (error) {
    ...
    static async getTestById(c) {
        try {
            ...
            const test = new Test(c.env.DB);
            const data = await test.getTestById(testId);
            if (data) {
                return c.json(data);
            } else {
    ...
    static async createTest(c) {
        try {
            const { testName, age } = await c.req.json();
            const newTest = new Test(c.env.DB, testName, age);
